...
for(const width of [390,1440])test(`atlas reveals real detail and preserves navigation at ${width}px`,async({page})=>{
 ...
 await page.goto('/login');await page.getByLabel('Email or username').fill('alice');await page.getByLabel('Password').fill('fixture-password');await page.getByRole('button',{name:'Sign in',exact:true}).click();await expect(page).toHaveURL(/\/team$/);
 ...
 await page.getByLabel('Explore a macro area').selectOption('P12');await page.getByRole('button',{name:'Show',exact:true}).click();
 await expect(page.getByRole('heading',{name:'P12',exact:true})).toBeVisible();
 await page.getByRole('button',{name:'← Back',exact:true}).click();
 await page.getByRole('button',{name:'Explore Aurelia',exact:true}).click();
 await expect(page.getByText('Districts and local places',{exact:true})).toBeVisible();
 ...
 await expect(page.getByText('Buildings, squares, and gardens',{exact:true})).toBeVisible();
 ...
 await page.getByRole('button',{name:'← Back',exact:true}).click();await expect(page.getByRole('heading',{name:'Old Aurelia',exact:true})).toBeVisible();
 await page.getByRole('button',{name:'Show the whole island',exact:true}).click();
 await expect(page.getByRole('heading',{name:'Pelotonia',exact:true})).toBeVisible();
 await expect(page.getByRole('button',{name:'Explore Cathedral building',exact:true})).toHaveCount(0);
 await page.getByRole('button',{name:'Earth context ↗'}).click();await expect(page.getByText(/precise Earth coordinates are still undecided/)).toBeVisible();
});
...
test('touch pinch zooms the atlas without opening a place',async({browser})=>{
 ...
 try{
  await page.goto('http://localhost:3100/login');await page.getByLabel('Email or username').fill('alice');await page.getByLabel('Password').fill('fixture-password');await page.getByRole('button',{name:'Sign in',exact:true}).click();await expect(page).toHaveURL(/\/team$/);
  await page.goto('http://localhost:3100/team/atlas');
  const map=page.getByRole('group',{name:/Map of Pelotonia/});await expect(map).toBeVisible();await map.scrollIntoViewIfNeeded();
