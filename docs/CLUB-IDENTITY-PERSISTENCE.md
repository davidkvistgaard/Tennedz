# Account-saved club identity (released)
...
Migration initially applied to nxhvaoonnvmvohqaxfdx (pelotonia-recovery-auth-test). After explicit owner approval it was applied to thacsxtnycmnnpjobgiv (Tennedz production) on 24 September 2026; see CLUB-IDENTITY-RELEASE.md. Supabase project listing confirmed isolation after initial automatic approval review misclassified the test project; retry with verified identity succeeded. SQL tests checked RLS/grants and two upserts in a rolled-back transaction. Production RLS and role grants were verified after the approved migration.
...
A separate release candidate based on the live visual package includes persistence without race-order migrations or Race Lab. The owner approved the additive migration and release; the production rebuild and live save/reload checks are complete. Roll back the application if necessary while retaining the additive table and saved choices.
