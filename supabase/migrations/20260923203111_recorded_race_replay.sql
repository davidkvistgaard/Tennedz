-- Additive. Existing results/feed remain intact and are never resimulated.
alter table public.event_division_runs add column if not exists replay jsonb;
alter table public.event_division_runs add constraint recorded_replay_v1 check (
  replay is null or (
    jsonb_typeof(replay)='object' and
    replay->>'version' is not distinct from '1' and
    jsonb_typeof(replay->'frames') is not distinct from 'array' and
    jsonb_typeof(replay->'roster') is not distinct from 'array' and
    jsonb_typeof(replay->'events') is not distinct from 'array' and
    jsonb_array_length(replay->'frames') between 2 and 1000 and
    jsonb_array_length(replay->'roster') between 16 and 160
  )
);
comment on column public.event_division_runs.replay is 'Complete immutable server-calculated replay v1. Playback cannot change the result. Null for historical text-only runs.';
...
begin
  ...
  for d in select value from jsonb_array_elements(p_output->'divisions') loop
    ...
    insert into public.event_division_runs(event_id,division_index,seed,engine_version,stage_snapshot,feed,results,replay)
      values(p_event,(d->>'index')::integer,d->>'seed',p_output->>'engine_version',p_output->'stage',d->'feed',d->'results',d->'replay');
