...
import { recordReplay } from "../race/replay.mjs";
...
function riderScoreFlat(r, weather, terrain = "flat") {
...
export function simulateStage({ stage, teamsWithRiders, seed, weather }) {
  ...
  const terrain = tags.includes("MOUNTAIN") ? "mountain" : tags.includes("HILLS") ? "hills" : tags.includes("COBBLES") ? "cobbles" : tags.includes("TT") ? "timetrial" : "flat";
  ...
  feed.push({
    km: 0,
    text: `Weather: ${w.condition} · ${w.temp_c}°C · wind ${w.wind_kph} km/h (${w.wind_dir}) · precipitation ${w.precipitation_mm} mm.`
  });
  ...
  for(const t of teamsWithRiders){
    if(t.orders){
      const helpers=t.riders.filter(r=>r.tactics?.role==="helper");
      const attackers=t.riders.filter(r=>r.tactics?.role==="attacker");
      feed.push({km:0,kind:"orders",rider_ids:t.riders.map(r=>r.id),text:`${t.name}: ${t.orders.plan === "captain" ? "racing for the captain" : t.orders.plan === "breakaway" ? "looking for a breakaway" : t.orders.plan === "conserve" ? "saving energy at the cost of speed" : "a balanced race plan"}. ${helpers.length} helpers support the captain; ${attackers.length} riders have freedom to attack.`});
    }
  }
  ...
  const breakGroup = [];
  if (rng() < breakChance) {
    const shuffled = riders.filter(r=>(r.raw.tactics?.attackWeight ?? 1)>0);
    for (let i = shuffled.length - 1; i > 0; i--) {
      const j = Math.floor(rng() * (i + 1));
      [shuffled[i], shuffled[j]] = [shuffled[j], shuffled[i]];
    }
    const priority=new Map(shuffled.map(r=>[r.rider_id,Math.pow(rng(),1/(r.raw.tactics?.attackWeight ?? 1))]));
    shuffled.sort((a,b)=>priority.get(b.rider_id)-priority.get(a.rider_id));
  ...
  if (breakGroup.length) {
    feed.push({ km: Math.min(8,dist*.1), kind:"attack", rider_ids:breakGroup.map(r=>r.rider_id), text: `${breakGroup.map(x => x.rider_name).join(", ")} attack and break away!` });
  } else {
    feed.push({ km: 10, text: "No one gets clear — the peloton stays together." });
  }

  if (hasSplits) {
    feed.push({ km: Math.min(60,dist*.48), kind:"split", text: "Crosswinds! The peloton splits into echelons as riders fight to stay in contact." });
  } else if (!calm) {
    feed.push({ km: Math.min(70,dist*.55), text: "The pace builds gradually, but the peloton remains in control." });
  } else {
    feed.push({ km: Math.min(75,dist*.6), text: "A quiet day — everyone seems to be saving energy for the finale." });
  }

  // Final 5 km: add more granular events
  const finalTexts = [
    "5 km to go — the teams begin fighting for position.",
    "4 km — the pace is high, and finding the right wheel matters.",
    "3 km to go — the race is approaching its decisive moment.",
    "2 km — nerves are rising as riders fight for space.",
    "1 km — now the sprint will decide it!"
  ];
  for (let i = 5; i >= 1; i--) {
    feed.push({ km: dist - i, kind:"finale", text: terrain === "flat" ? finalTexts[5-i] : `${i} km to go — the finale will be decided on ${terrain === "mountain" ? "the mountain roads" : terrain === "hills" ? "the hills" : terrain === "cobbles" ? "the cobbles" : "the time trial course"}.` });
  }
  ...
  const perf = riders.map(rr => {
    const flatScore = riderScoreFlat(rr.raw, w, terrain)*(rr.raw.tactics?.pace ?? 1);
    const sprScore = (terrain === "flat" ? sprintScore(rr.raw, w) : riderScoreFlat(rr.raw,w,terrain))*(rr.raw.tactics?.finish ?? 1);
  ...
  if (breakGroup.length) {
    ...
    const chaseStr = perf.slice(0, 16).reduce((s, rr) => s + Number(rr.raw.strength ?? 0) + Number(rr.raw.endurance ?? 0), 0) / Math.min(16, perf.length);
    ...
    feed.push({
      km: dist - 12,
      kind: breakSurvives ? "break-holds" : "catch", rider_ids:breakGroup.map(r=>r.rider_id),
      text: breakSurvives
        ? "The breakaway holds on into the finale against the odds!"
        : "The peloton organises the chase — the breakaway is caught."
    });
  }
  ...
  feed.push({
    km: dist, kind:"finish", rider_ids:[winner.rider_id],
    text: `Finish! ${winner.rider_name} (${winner.team_name}) takes the win.`
  });
  ...
  for (const rr of rest) {
    ...
    if (hasSplits && rng() < 0.35) { t += splitGap; splitIds.push(rr.rider_id); }
  ...
  // Feed must progress chronologically and stay within the actual route.
  const splitEvent=feed.find(line=>line.kind==='split');
  if(splitEvent){
    splitEvent.rider_ids=splitIds;
    splitEvent.text=splitIds.length
      ? `${splitIds.length} riders lose contact in ${Number(w.wind_kph)>=20?'the wind':'the difficult conditions'}. The peloton is split.`
      : 'The peloton stays together through the difficult conditions.';
    if(!splitIds.length)splitEvent.kind='race';
  }
  feed.forEach(line => { line.km = clamp(line.km, 0, dist); });
  feed.sort((a,b) => a.km-b.km);
  const replay=recordReplay({stage,weather:w,riders,results,breakIds:breakGroup.map(r=>r.rider_id),breakSurvives,splitIds,splitGap,feed});
  return { feed, results, replay };
}
