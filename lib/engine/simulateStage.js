...
function makeMessage(type, ctx) {
  const { km, rng, gap, kmLeft, teamName } = ctx;
  ...
  const variants = {
    ...
    helpers_drop_back: [
      `${km} km: Hjælperyttere falder tilbage for at hjælpe kaptajnen tilbage.`,
      `${km} km: Kaptajnen er i problemer – holdkammeraterne går i redningsmode.`,
      `${km} km: Holdet reorganiserer sig for at begrænse tidstabet til kaptajnen.`
    ],
    break_caught: [
...
export function simulateStage({
  ...
  function drainEnergy(groupType, rider, terrain, effort, plan) {
    ...
    // Sprinter "saver sig" i feltet (bruger mindre energi i peloton)
    if (groupType === "peloton" && roles.sprinter && rider.rider_id === roles.sprinter) {
      base *= 0.82;
    }

    // Rouleur "trækker" hårdere når der jagtes (koster energi)
    // (kun relevant hvis der er et udbrud foran)
    if (groupType === "peloton" && breakaway && roles.rouleur && rider.rider_id === roles.rouleur) {
      base *= 1.18;
    }
  ...
  // Collective pull index: strength+endurance+energy + role bonus
  function pelotonPullIndex(terrain) {
    const ids = peloton.rider_ids;
    if (!ids.length) return 1.0;

    // sample up to 30 riders for speed
    ...
    for (const id of sample) {
      ...
      // ROLE: rouleur counts more in pull
      const roles = rolesForTeam(r.team_id);
      if (roles.rouleur && r.rider_id === roles.rouleur && terr !== "mountain") {
        pull *= 1.25;
      }

      // team focus influences contribution a bit
  ...
  // Pick a "leading chase team" for flavor text
  function pickChaseLeaderTeam(terrain, kmLeft) {
    ...
    for (const t of teams) {
      ...
      ensureTeamState(team_id);
      const plan = activePlanByTeam.get(team_id);
      const chase = teamChaseIntensity(team_id, terrain, kmLeft);
      ...
      if (roles.rouleur && peloton.rider_ids.includes(roles.rouleur)) {
        ...
        if (rr) {
          roleBonus = 0.15 * ((clamp(s(rr.skills, "Strength"), 0, 100) + clamp(s(rr.skills, "Endurance"), 0, 100)) / 200);
        }
      }
      const score = chase + roleBonus + (plan?.focus === "sprint" ? 0.10 : 0) + (Math.random() * 0.02);
  ...
  function maybeChaseOrCatch(km, seg, kmLeft) {
    ...
    // Named chase message sometimes
    if (rng() < 0.10) {
      const leadTeamId = pickChaseLeaderTeam(terr, kmLeft);
      const teamName = leadTeamId ? (teamNameById.get(leadTeamId) || "Et hold") : "Et hold";

      const type = leadTeamId ? "peloton_chasing_named" : "peloton_chasing_generic";
      addFeed(km, "peloton_chasing", makeMessage(type, { km, rng, teamName }), {
  ...
  // NEW: Captain protection — if captain is in chase group, helpers drop back and reduce gap
  function captainProtectionTick(km) {
    if (!chase) return;
    ...
    for (const t of teams) {
      ...
      const captainInChase = chase.rider_ids.includes(captainId);
      if (!captainInChase) continue;

      // Move up to 2 helpers from peloton to chase (exclude sprinter + captain)
      const helpers = peloton.rider_ids
        .map((id) => riderById.get(id))
        .filter((r) => r && r.team_id === team_id && r.rider_id !== captainId && r.rider_id !== roles.sprinter)
      ...
      const toMove = helpers.slice(0, 2);
      ...
      for (const r of toMove) {
        ...
        // little energy hit for the move
        r.energy = clamp(r.energy - (2 + rng() * 4), 0, 100);
      }

      // helping reduces the chase gap slightly
      chase.gap_sec = Math.max(peloton.gap_sec + 6, chase.gap_sec - (3 + rng() * 6));

      if (rng() < 0.6) {
        addFeed(km, "helpers_drop_back", makeMessage("helpers_drop_back", { km, rng }), {
          team_id,
          team_name: teamNameById.get(team_id) || team_id,
          captain_id: captainId,
          helpers: toMove.map((x) => x.rider_id)
        });
      }
    }
  }
  ...
  for (let km = 0; km <= Math.floor(distance); km++) {
    ...
    // NEW: captain protection step
    captainProtectionTick(km);
  ...
  for (const g of groupsAtFinish) {
    ...
    ids.sort((a, b) => {
      ...
      // sprinter gets a slight finale edge if he conserved energy
      const rolesA = rolesForTeam(ra.team_id);
      const rolesB = rolesForTeam(rb.team_id);
      const sprintBonusA = rolesA.sprinter && ra.rider_id === rolesA.sprinter ? 1.06 : 1.0;
      const sprintBonusB = rolesB.sprinter && rb.rider_id === rolesB.sprinter ? 1.06 : 1.0;

      const pa = riderPower(ra.skills, "finale", sprintBonusA) + 0.15 * ra.energy;
      const pb = riderPower(rb.skills, "finale", sprintBonusB) + 0.15 * rb.energy;
      return pb - pa;
    });
