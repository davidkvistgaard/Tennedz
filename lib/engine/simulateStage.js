...
export function simulateStage({ stage, teamsWithRiders, seed, weather }) {
  ...
  if (breakGroup.length) {
    feed.push({ km: Math.min(8,dist*.1), kind:"attack", rider_ids:breakGroup.map(r=>r.rider_id), text: `${breakGroup.map(x => x.rider_name).join(", ")} går i udbrud!` });
  } else {
    feed.push({ km: 10, text: "Ingen kommer rigtigt afsted — feltet holder det samlet." });
  }

  if (hasSplits) {
    feed.push({ km: Math.min(60,dist*.48), kind:"split", text: "Sidevind! Feltet splittes i vifter, og flere ryttere kæmper for at sidde med." });
  } else if (!calm) {
    feed.push({ km: Math.min(70,dist*.55), text: "Tempoet stiger gradvist, men feltet ser kontrolleret ud." });
  } else {
    feed.push({ km: Math.min(75,dist*.6), text: "En rolig dag — alle virker til at spare kræfter til finalen." });
  }
  ...
  const finalTexts = [
    ...
    "3 km igen — løbet nærmer sig sin afgørelse.",
    "2 km — nervøst, og der bliver kæmpet om pladsen.",
    "1 km — nu er det sprinten der afgør det!"
  ];
  for (let i = 5; i >= 1; i--) {
    feed.push({ km: dist - i, kind:"finale", text: terrain === "flat" ? finalTexts[5-i] : `${i} km igen — finalen afgøres på ${terrain === "mountain" ? "bjergvejene" : terrain === "hills" ? "bakkerne" : terrain === "cobbles" ? "brostenene" : "enkeltstartsruten"}.` });
  }
  ...
  feed.push({
    ...
    text: `Mål! ${winner.rider_name} (${winner.team_name}) tager sejren.`
  });
  ...
  if(splitEvent){
    ...
    splitEvent.text=splitIds.length
      ? `${splitIds.length} ryttere mister kontakten i ${Number(w.wind_kph)>=20?'vinden':'det vanskelige føre'}. Feltet er delt.`
      : 'Feltet holder sammen gennem det vanskelige føre.';
