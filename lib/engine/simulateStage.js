...
function makeMessage(type, ctx) {
  ...
  const variants = {
    attack_started: [
      `${km} km: Et ryk i feltet – en rytter prøver lykken!`,
      `${km} km: Der bliver angrebet fra feltet!`,
      `${km} km: Tempoet stiger, og der bliver sat et angreb ind.`
    ],
    break_formed: [
    ...
    peloton_chasing: [
      ...
      `${km} km: Der bliver kørt for at hente udbruddet.`
    ],
    break_caught: [
    ...
    crosswind_split: [
      `${km} km: Sidevind! Feltet splittes og der opstår store huller.`,
      `${km} km: En brutal sidevindssituation – der bliver revet over!`,
      `${km} km: Vinden gør ondt. Grupperne former sig.`
    ],
    cobbles_chaos: [
      `${km} km: Paveerne skaber kaos – positionering er altafgørende.`,
      `${km} km: Ind på pave-sektoren – rytterne kæmper om hjul.`,
      `${km} km: Paveerne sprænger det hele i stykker.`
    ],

    // Finale – state-aware tekster (besluttet i main loop)
    finale_pos_sprint: [
    ...
    finale_train_sprint: [
      `${km} km: 3 km igen. Tog og leadouts samler op – tempoet eksploderer!`,
      `${km} km: Leadout-togene tager over med 3 km igen.`
    ],
    finale_train_break: [
      `${km} km: 3 km igen – udbruddet giver alt. De kan næsten se målet!`,
      `${km} km: 3 km igen. Udbruddet er stadig foran – det bliver et kapløb mod stregen.`
    ],
    finale_train_split: [
    ...
    finale_launch_sprint: [
      ...
      `${km} km: Nu åbner sprinterne med 1 km igen!`
    ],
    finale_launch_break: [
      `${km} km: 1 km igen – udbruddet går i total rød zone mod stregen!`,
      `${km} km: 1 km igen. Udbruddet spurter om sejren!`
    ],
    finale_launch_split: [
      `${km} km: 1 km igen – frontgruppen åbner. Det bliver en lille gruppe-spurt!`,
...
export function simulateStage({
  ...
  function snapshot(km) {
    ...
    snapshots.push({ km, state: { km, groups: groupSummary } });
  }
  ...
  function pickAttackersFromPeloton(terrain, count) {
    ...
    candidates.sort((a, b) => {
      const pa = riderPower(a.skills, terrain, 1);
      const pb = riderPower(b.skills, terrain, 1);
      return pb - pa;
    });
  ...
  function maybeChaseOrCatch(km, seg, kmLeft) {
    ...
    if (rng() < 0.08) {
      addFeed(km, "peloton_chasing", makeMessage("peloton_chasing", { km, rng }), { chase_strength: round1(chaseStrength) });
    }
  ...
  for (let km = 0; km <= Math.floor(distance); km++) {
    ...
    // crosswind & break (undgå store nye angreb i sidste 5 km)
    ...
    // --- Finale events afhængigt af status ---
    ...
    if (kmLeft === 5 && !finaleFlags.pos) {
      ...
      addFeed(km, type, makeMessage(type, { km, rng }), {
        km_left: 5,
        status: { has_break: hasBreak, has_split: hasSplit, break_gap_sec: hasBreak ? round1(peloton.gap_sec - breakaway.gap_sec) : null }
      });
    }
  ...
  // winner event (payload for future name mapping)
