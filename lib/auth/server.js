...
export async function sessionClient() {
  ...
  if (!url || !key) throw new AuthError("AUTH_CONFIGURATION", "Sign-in is not configured in this environment yet.", 503);
...
export function databaseClient() {
  ...
  if (!url || !key) throw new AuthError("DATA_CONFIGURATION", "Team data is not configured in this environment yet.", 503);
...
export async function requireUser() {
  ...
  if (error && (!error.status || error.status >= 500)) throw new AuthError("AUTH_UNAVAILABLE", "The sign-in service is unavailable. Please try again.", 503);
  if (error || !data?.user) throw new AuthError("UNAUTHENTICATED", "Please sign in.");
...
export async function requireTeam() {
  ...
  if (error) throw new AuthError("TEAM_UNAVAILABLE", "Could not load your team. Please try again.", 503);
...
export function requireGameWrites() {
  if (process.env.RECOVERY_ALLOW_GAME_WRITES !== "true") throw new AuthError("GAME_READ_ONLY", "The game is temporarily read-only during recovery.", 503);
}
...
export async function suspendRaceWrite(req) {
  try {
    ...
    throw new AuthError("RACE_RECOVERY_PENDING", "Race processing and time changes will open after the game cycle has been verified.", 503);
  } catch (error) { return authFailure(error); }
}
export function authFailure(error) {
  const known = error instanceof AuthError;
  return NextResponse.json({ ok: false, code: known ? error.code : "SERVICE_UNAVAILABLE", error: known ? error.message : "The service is unavailable. Please try again." }, {
