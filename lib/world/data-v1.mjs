...
const regionSeeds = [
  ...
  ['REG-NORTHERN-PLATEAU','P','Northern Plateau',null,'Elevated transitional country east of the Northern Highlands and north/northeast of the Central Plains.', {
    elevation: range(600,1400,'m'),
    populationDensity: 'relatively low',
    landCover: { dominant: ['open grassland','heath'], scattered: ['forest'] },
    landforms: ['broad valleys','ridges','upland lakes','river headwaters'],
    climate: { relativeToWesternHighlands: 'cooler and substantially drier', winter: 'regular snow', summer: 'generally dry; mild-to-warm at lower elevations' },
  }],
];
const classifications = Object.fromEntries(regionSeeds.map(([id,code]) => [code,id]));
const grid = createGrid(classifications);
const regions = regionSeeds.map(([id,code,name,population,description,properties = {}]) => object(id,name,'region','WORLD-001',1,
  { environmentCode: code, population: population === null ? null : approximate(population,'people'), physicalStatus: 'fixed', ...properties },
  {description, gridCells: grid.filter(c => c.properties.environmentCode === code).map(c => c.name),
    ...(code === 'P' ? {relations:[{kind:'east_of',targetId:'REG-002'},{kind:'north_northeast_of',targetId:'REG-006'}]} : {})}));
...
export const worldV1 = {
  ...
  issues: [
    {id:'SOURCE-001',status:'resolved',description:'Owner clarified the omitted P classification as Northern Plateau (REG-NORTHERN-PLATEAU). All 11 original P cells are defined; their positions are unchanged.',objectIds:['REG-NORTHERN-PLATEAU',...grid.filter(c => c.properties.environmentCode === 'P').map(c => c.id)]},
    {id:'SOURCE-002',description:'The original 12 regional estimates sum to approximately 12,100,000. Northern Plateau has low density but no assigned population; the regional distribution needs future reconciliation, not an invented addition or redistribution. City estimates are subsets.',objectIds:['WORLD-001','REG-NORTHERN-PLATEAU']},
