...
// Verbatim source classification. P is not defined in the supplied legend.
  ...
  return ENVIRONMENT_ROWS.flatMap((row, y) => [...row].map((code, x) => {
    ...
    return {
      ...
      relations: [], properties: { column: GRID.columns[x], row: y+1, environmentCode: code,
        classificationStatus: code === 'P' ? 'unresolved' : 'defined', regionId: classifications[code] ?? null },
    };
  }));
}
