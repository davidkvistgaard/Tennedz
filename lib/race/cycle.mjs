...
export const ENGINE_VERSION = "recovery-one-day-1";
...
export function seedPower(r) {
  return skills.reduce((sum,k) => sum+Number(r[k] ?? 0),0)/9 * (0.85+Number(r.form ?? 50)/100*0.30) * (1-Number(r.fatigue ?? 0)/100*0.20);
}
...
export function buildRace(snapshot) {
  ...
  if (event?.kind!=="one_day" || !stage || !Number.isFinite(Number(stage.distance_km)) || stage.distance_km<20 || stage.distance_km>400) throw new Error("Endagsløbet mangler en gyldig rute på 20–400 km.");
  if (!Array.isArray(teams) || teams.length<2 || teams.length>400) throw new Error("Et endagsløb kræver 2–400 hold.");
  const allSelected = new Set();
  const prepared = teams.map(t => {
    const ids=t.entry.selected_riders;
    if (!Array.isArray(ids) || ids.length!==8 || new Set(ids).size!==8 || !ids.includes(t.entry.captain_id)) throw new Error("En tilmelding mangler otte forskellige ryttere eller en kaptajn.");
    const riders=ids.map(id => t.riders.find(r=>r.id===id));
    for (const r of riders) {
      if (!r || allSelected.has(r.id) || (event.gender && r.gender!==event.gender) || (r.injury_until && r.injury_until>game_date)) throw new Error("En tilmelding har ugyldige, skadede eller fremmede ryttere.");
      if (skills.some(k=>!Number.isFinite(Number(r[k] ?? 0)))) throw new Error("Ugyldige rytteregenskaber.");
      allSelected.add(r.id);
    }
    const eligible=t.riders.filter(r=>!event.gender || r.gender===event.gender);
    return {...t, seed_power:eligible.map(seedPower).sort((a,b)=>b-a).slice(0,16).reduce((a,b)=>a+b,0),
  ...
  const divisions=groups.map((group,i)=>{
    ...
    if (sim.results.length!==group.length*8) throw new Error("Motoren returnerede et ufuldstændigt resultat.");
    ...
    return {index,seed:divSeed,multiplier,teams:teamResults,results,feed:sim.feed};
  });
