...
export function normalizeRoute(stage = {}) {
  ...
  if (!Number.isFinite(distance) || distance < 20 || distance > 400)
    throw new Error("The route must be between 20 and 400 km.");
  const points = (
  ...
  const keypoints = (Array.isArray(stage.keypoints) ? stage.keypoints : [])
    ...
    .map((k) => ({
      ...
      label: String(k.label || "Route point").slice(0, 100),
    }));
  if (!keypoints.some((k) => k.kind === "FINISH"))
    keypoints.push({ km: distance, kind: "FINISH", label: "Finish" });
...
export function routeAdvice(route) {
  const advice = {
    flat: ["sprint", "flat", "Sprinting and positioning in the finale"],
    hills: ["hills", "endurance", "Repeated accelerations on the hills"],
    mountain: ["mountain", "endurance", "Climbing strength and endurance"],
    cobbles: ["cobbles", "strength", "Cobbles and the ability to sustain the pace"],
    timetrial: ["timetrial", "endurance", "Time trialling and a steady pace"],
  };
