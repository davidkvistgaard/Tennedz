...
// Reject forged/unallocated choices on writes; fallback validation is display-only.
export function validKitSelection(value, teamId) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  const keys = Object.keys(value);
  const choices = teamKitOptions(teamId);
  return keys.length === 2 && keys.includes("palette") && keys.includes("pattern")
    && choices.palettes.includes(value.palette) && choices.patterns.includes(value.pattern);
}
